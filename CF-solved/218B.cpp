...
using namespace std;
int main(){
    ll prsn,pln; cin>> prsn>>pln;
    vll v(pln),mn(pln),mx(pln);
    fi(pln){
    	cin>>v[i];
    	mn[i]=v[i];
    	mx[i]=v[i];
    ...
    srt(mn); rsrt(mx);
   	...
   	while(pr--){
   		...
   		// cout<<mx[temp] sp;
   		mx[temp]--;
   		if(mx[temp]<mx[temp+1] && temp+1 != pln) temp++;
   		if(mx[0]>mx[temp]) temp=0;

   	}
   	cout<<mxans sps mnans;
}